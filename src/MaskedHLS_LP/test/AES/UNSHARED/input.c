...
void G16_mul(int x0, int y0, int *o0)
{
    ...
    // axorb_1 = a1 ^ b1;
    // cxord_1 = c1 ^ d1;
    G4_mul(axorb_0, cxord_0, &e0);//here
    int e01, e11;
    G4_scl_N(e0, &e01);

    G4_mul(a0, c0, &p0_0);//here
    p0 = p0_0 ^ e01;
    // p1 = p1_0 ^ e11;

    G4_mul(b0, d0, &q0_0);//here
    q0 = q0_0 ^ e01;
    // q1 = q1_0 ^ e11;
    int p0ls2;
    p0ls2 = p0 << 2;
...
void G16_inv(int x0, int *y0)
{
    ...
    G4_scl_N(c0_0, &c0);
    G4_mul(a0, b0, &d0);//here
    c0xord0 = c0 ^ d0;
    // c1xord1 = c1 ^ d1;

    G4_sq(c0xord0, &e0); // really inverse, but same as square
    G4_mul(e0, b0, &p0);//here
    G4_mul(e0, a0, &q0);//here
    int p0ls2;
    p0ls2 = (p0 << 2);
    // p1ls2 = (p1 << 2);
    *y0 = (p0ls2 | q0);
