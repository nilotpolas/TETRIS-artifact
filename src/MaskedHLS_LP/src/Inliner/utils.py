...
#             # print(f"[DEBUG]: function name : {functionName}")
#             parameters = functionInfo[functionName]['parameters']
#             # print(f"[DEBUG]: function parameters : {parameters}")
#             localVariables = functionInfo[functionName]['localVariables']
#             # print(f"[DEBUG]: function local varibales : {localVariables}")
...
#             # print(f"len of argument : {len(arguments)}")
#             # print(f"len of parameter : {len(parameters)}")
...
def extractFunctionInfo(node,functionInfo):
    ...
    # Note: Variable renaming happens here, but the unique numerical suffix is added in processDependantFunctions
    for index in range(len(localVariables)):
        localVariables[index] += ('_' + function_name)
    ...
    # Initialize global counter if not present (Shared across all functions)
    if 'global_gadget_counter' not in functionInfo:
        functionInfo['global_gadget_counter'] = 0
...
def processDependantFunctions(node,functionInfo,isTopModule):
    ...
    for blockItem in functionBody.block_items:
        ...
        if isinstance(blockItem,c_ast.FuncCall):
            ...
            # ------------------------------------------------------------------
            # CHANGED LOGIC: Global Counter for Gadgets (HPC/AND)
            # ------------------------------------------------------------------
            # If the function is a Gadget (starts with HPC or contains 'and'), 
            # we use the Global Counter. Otherwise, we use the local 'timesCalled'.
            
            # 1. Initialize Global Counter if missing
            if 'global_gadget_counter' not in functionInfo:
                functionInfo['global_gadget_counter'] = 0
            
            # 2. Check if it is a gadget
            is_gadget = functionName.startswith("HPC") or "and" in functionName.lower()
            
            current_id_suffix = 0

            if is_gadget:
                # Use Global Counter
                current_id_suffix = functionInfo['global_gadget_counter']
                functionInfo['global_gadget_counter'] += 1
            else:
                # Use Local Counter (for non-gadget helper functions)
                current_id_suffix = functionInfo[functionName]['timesCalled']
                functionInfo[functionName]['timesCalled'] += 1
            # ------------------------------------------------------------------
            ...
            for localVariable,localVariableType in zip(localVariables,localVariablesType):
                # Append the calculated Global ID to the variable name
                # e.g., u00_HPC3 + "0" -> u00_HPC30
                # e.g., u00_HPC1 + "1" -> u00_HPC11
                declNode = getDeclarationNode(localVariable + str(current_id_suffix), localVariableType)
                localVariablesMap[localVariable] = localVariable + str(current_id_suffix)
                bodyNodes.append(declNode)
...
def generate_c_file(ast,filename):
    """
    @brief Generates C code from the provided AST and writes it to a file.
    """
    generator = c_generator.CGenerator()
    with open(filename, 'w') as output_file:
        print(generator.visit(ast), file=output_file)