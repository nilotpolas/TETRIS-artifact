...
def writeVerilogToFile(funcName, functions, internalVariables, functionCalls, filename,width):
    ...
    with open(filename, 'w') as file:
        ...
        ports = functions[funcName]["inputs"] + functions[funcName]["outputs"]
        ports_lines = ",\n    ".join(ports)
        print("    " + ports_lines, file=file)
        print(");", file=file)
        ...
        for op in functions[funcName]["outputs"]:
            # Correctly handle reg or regPR for outputs
            if internalVariables[op]["type"] in ["reg", "regPR"]:
        ...
        for key in internalVariables.keys():
            if key not in functions[funcName]["outputs"]:
                # FIX: Verify regPR is printed as 'reg', not 'regPR'
                v_type = internalVariables[key]["type"]
                if v_type == "regPR": 
                    v_type = "reg"
                
                print("    " + v_type + width + key + ";", file=file)
        ...
        # FIX: Check if ANY variable is regPR or if regPresent is True
        has_registers = functions[funcName]["regPresent"]
        if not has_registers:
            # Fallback: scan variables just in case the parser missed setting the flag
            for key in internalVariables:
                if internalVariables[key]["type"] == "regPR":
                    has_registers = True
                    break
        ...
        if has_registers:
            ...
            for key in internalVariables.keys():
                # FIX: Check 'key', do not use 'op' here
                if internalVariables[key]["type"] in ["reg", "regPR"]:
