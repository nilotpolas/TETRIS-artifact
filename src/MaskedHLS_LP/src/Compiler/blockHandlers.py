...
def reconstruct_expression(node,internalVariables):
    ...
    if isinstance(node, c_ast.BinaryOp):
        ...
        if node.name.name == "reg" or node.name.name == "regPR":
...
def handleDeclBlock(node,internalVariables):
    ...
    if isinstance(node.init,c_ast.FuncCall) and (node.init.name.name == "reg" or node.init.name.name == "regPR"):
...
def handleAssignmentBlock(node,internalVariables):
    ...
    if isinstance(node.rvalue,c_ast.FuncCall) and (node.rvalue.name.name == "reg" or node.rvalue.name.name == "regPR"):
