...
def add_dummy_node_fan_out(dfgCopy: rx.PyDiGraph,nodesInfo,nameIndexMap):
    ...
    for node_index in existing_nodes:
        ...
        if (isinstance(node, ASTGraphNode)):
            ...
            # print(f"node info : {nodeInfo}")
            # If the rvalue is annotated using CrossDomainOp, it means a dummy node
            # has to be added after the current node
            inputRegisters = nodeInfo["inputRegisters"]
            
            if inputRegisters:
                # Map input register names to node indices, but skip names missing
                # from nameIndexMap to avoid KeyError (e.g. external/random inputs).
                missing = [r for r in inputRegisters if r not in nameIndexMap]
                # if missing:
                    # print(f"[RegBalancer][WARN] skipping {len(missing)} input register(s) absent from nameIndexMap: {missing[:8]}")
                inputRegisters = [nameIndexMap[reg_name] for reg_name in inputRegisters if reg_name in nameIndexMap]
...
def is_reg_call(node):
    ...
    if (isinstance(node, cast.FuncCall) and ( node.name.name == 'regPR'  or node.name.name == 'reg') ):
        return True
    return False
