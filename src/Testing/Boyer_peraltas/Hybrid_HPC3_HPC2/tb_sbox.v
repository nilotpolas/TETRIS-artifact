...
module tb_sbox_updated();
    ...
    reg [63:0] rands; // Expanded to 64 bits to cover the 46 random ports
    ...
    sbox dut (
        ...
        // Map exactly 46 random ports from the sbox.v module incrementally
        .rand_4(rands[0]),   .rand_10(rands[1]),  .rand_35(rands[2]),  .rand_16(rands[3]),
        .rand_19(rands[4]),  .rand_3(rands[5]),   .rand_2(rands[6]),   .rand_30(rands[7]),
        .rand_7(rands[8]),   .rand_13(rands[9]),  .rand_9(rands[10]),  .rand_21(rands[11]),
        .rand_43(rands[12]), .rand_39(rands[13]), .rand_8(rands[14]),  .rand_25(rands[15]),
        .rand_18(rands[16]), .rand_27(rands[17]), .rand_61(rands[18]), .rand_12(rands[19]),
        .rand_15(rands[20]), .rand_45(rands[21]), .rand_26(rands[22]), .rand_65(rands[23]),
        .rand_29(rands[24]), .rand_55(rands[25]), .rand_14(rands[26]), .rand_57(rands[27]),
        .rand_31(rands[28]), .rand_37(rands[29]), .rand_47(rands[30]), .rand_11(rands[31]),
        .rand_49(rands[32]), .rand_33(rands[33]), .rand_59(rands[34]), .rand_6(rands[35]),
        .rand_1(rands[36]),  .rand_20(rands[37]), .rand_41(rands[38]), .rand_17(rands[39]),
        .rand_51(rands[40]), .rand_67(rands[41]), .rand_5(rands[42]),  .rand_53(rands[43]),
        .rand_23(rands[44]), .rand_63(rands[45]),
    ...
    initial begin
        ...
        for (i = 0; i < 256; i = i + 1) begin
            ...
            // Wait several clock cycles to allow the data to propagate through the deep
            // retimed pipeline while supplying fresh randomness continuously.
            for (j = 0; j < 6; j = j + 1) begin
                @(negedge clk);
                // $random returns 32 bits. Appending two covers 64 bits (plenty for 46 ports)
                rands = {$random, $random};
            end
            #50;
            
            // Sample exactly after pipeline has securely flushed (safe post-neg-edge check)
            if (out_unmasked !== sbox_golden[i]) begin
                $display("ERROR at Input 0x%02X | Expected: 0x%02X | Got: 0x%02X", i, sbox_golden[i], out_unmasked);
                errors = errors + 1;
            end else begin
                // Uncomment to trace passing indices:
