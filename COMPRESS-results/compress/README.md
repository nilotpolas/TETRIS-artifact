...
2. **COMPRESS source code** (`compress/`) — the version of COMPRESS used to generate those results, minus the NanGate45 PDK (which TETRIS ships elsewhere).
...
│
├── compress/             # COMPRESS source code, our version 
...
### Go to comparision and run the script to link the library from TETRIS folder
```
cd ../comparision
bash setup.sh
```
...
- **NanGate45 PDK** (~6.5 MB) — TETRIS ships this library elsewhere; symlink (See `../comparision/setup.sh`) or download from the COMPRESS repo as needed.
- **Verilog outputs** (~977 MB in the original archive) — regeneratable from the source + stats. Reviewers rarely need this.
- **Solver info** — internal state of COMPRESS's CP-SAT solver; not needed for validation.
...
Both the derived `stats.json` files in `compress_logs/` and the source code in `compress/` are redistributed here under the same terms.