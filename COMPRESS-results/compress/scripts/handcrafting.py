...
This is an adaptation (to fit the CLI of compress.py) of the tool described in:

Charles Momin, Gaëtan Cassiers, and François-Xavier Standaert. Handcrafting:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Automated generation of HPC verilog netlist"
    )
    parser.add_argument(
        "--circuit",
        type=Path,
    ...
    parser.add_argument(
        "--verilog-module", type=str, help="Name of the generated module file. "
    )
    parser.add_argument("--out", type=Path, required=True, help="Output file.")
    parser.add_argument(
        "--outstats", type=Path, required=True, help="Output stats json file."
    )
    parser.add_argument(
        "--gen-enable",
        action="store_true",
...
def parse_line(line,out_map):
    ...
    for e in zip(idstrs, ops):
        ...
        if "input(" in op:
            ...
            list_of_node.append([idstr, nodev, False, False])
        else:
...
def parse_circuit(filename):
    ...
    for l in gtxt_lines:
        ...
        if l.startswith("INPUTS"):
            for inp in l.split()[1:]:
                cleaned_lines.append(f"{inp} = input(d, data)")
        elif l.startswith("OUTPUTS"):
            for out in l.split()[1:]:
                cleaned_lines.append(f"{out} = output(gen__{out})")
    ...
    if vNode.time is None:
        ...
        if verbose:
            ...
            if nop.src0 is not None:
                print(
                    "-Src0 '{}' : {} (lat: {})".format(
                        nop.src0, circuits[nop.src0].time, nop.lat[0]
                    )
                )
            if nop.src1 is not None:
                print(
                    "-Src1 '{}' : {} (lat: {})".format(
                        nop.src1, circuits[nop.src1].time, nop.lat[1]
                    )
                )
...
def recur_add_delay_step_opin(op, idsrc, sig2add, circuit, ntype="data"):
    ...
    if sig2add in circuit.keys():
        ...
        recur_add_delay_step_opin(
            circuit[sig2add].op, 0, nextsig2add, circuit, ntype=ntype
        )
...
def fix_output_latency(circuit, outs):
    ...
    for ei, e in enumerate(time_output):
        ...
        if d2add > 0:
            ...
            recur_add_delay_step_opin(circuit[outs[ei]].op, 0, lds_name, circuit)
    # Recompute timing
    reset_latency_circuit(circuit)
    for o in outs:
    ...
    if circuit[idstr].op is None:
        ...
        if circuit[idstr].op.src0 is not None:
            rec_list_gate = merge_node_list(
                rec_list_gate, recur_gatelist_order(circuit[idstr].op.src0, circuit)
            )
        if circuit[idstr].op.src1 is not None:
            rec_list_gate = merge_node_list(
                rec_list_gate, recur_gatelist_order(circuit[idstr].op.src1, circuit)
            )
...
def create_gatelist_order(circuit, outs, ins):
    list_gate = []
    for out in outs:
        list_gate = merge_node_list(recur_gatelist_order(out, circuit), list_gate)
    return list_gate
...
def create_mux_ctrl_tree_order(circuit):
    ...
    for mctrl in mux_ctrl_id:
        list_gate = merge_node_list(recur_gatelist_order(mctrl, circuit), list_gate)
    return list_gate


def create_verilog_template_entry(
    var, iotype, def_clk="clk", def_en="enable", def_rst="rst", gen_en=False
):
    if iotype == "input":
        ...
        if var.op.op == "id":
            return "assign {} = {};\n".format(var.id, var.op.src0)
        elif var.op.op == "not":
            return "assign {} = !{};\n".format(var.id, var.op.src0)
        elif var.op.op == "add":
            return "assign {} = {} ^ {};\n".format(var.id, var.op.src0, var.op.src1)
        elif var.op.op == "and":
            return "assign {} = {} & {};\n".format(var.id, var.op.src0, var.op.src1)
        elif var.op.op == "mux":
            return "assign {} = {} ? {} : {};\n".format(
                var.id, var.op.ctrlsig0, var.op.src0, var.op.src1
            )
        elif var.op.op == "notp*":
            return """MSKinv #(.d({}))
inv_{} (
    ...
    .out({})
                ...
                var.lenstr, var.id, var.op.src0, var.id
            )
...
xorhpc2_{} (
    ...
    .out({})
                ...
                var.lenstr, var.id, var.op.src0, var.op.src1, var.id
            )
...
regen_{} (
    ...
    .out({})
                ...
                var.lenstr, var.id, def_clk, def_en, var.op.src0, var.id
            )
...
regen_{} (
    ...
    .out({})
                ...
                var.lenstr, var.id, def_clk, var.op.src0, var.id
            )
...
ctrl_reg_{} (
    ...
    .out({})
                ...
                var.lenstr, var.id, def_clk, def_en, def_rst, var.op.src0, var.id
            )
...
ctrl_reg_{} (
    ...
    .out({})
                ...
                var.lenstr, var.id, def_clk, def_rst, var.op.src0, var.id
            )
        elif var.op.op == "muxp*":
            return """MSKmux #(.d({}))
muxp_{} (
    ...
    .out({})
                ...
                var.lenstr, var.id, var.op.ctrlsig0, var.op.src0, var.op.src1, var.id
            )
        else:
...
def create_top_verilog_interface(name_module, circuit, outs, ins, gen_enable):
    ...
    header_lines = [
        ...
        "`endif",
        "module {} #(parameter d=`DEFAULTSHARES) (".format(name_module),
        "    {}".format(io_verilog),
        ");",
    ]
    return "\n".join(header_lines)
...
def create_validity_pipeline(circuit, outs, ins):
    ...
    if global_latency > 0:
        ...
        in_var = NodeVar(id_valid_in, None, 0, "control", "1")
        ...
        add_delay_opin(
            circuit[id_valid_out].op, 0, global_latency, circuit, ntype="control"
        )
...
def create_rnd_connection_string(length_used, btime, n_rnd_am):
    return "{}[{} +: {}]".format(
        create_rnd_bus_string(btime), create_len_string(length_used), n_rnd_am
    )
...
def connect_randomness_bus(circuit, bus_time, bus):
    ...
    for nodeid in bus["nodes"]:
        ...
        circuit[nodeid].op.rnd_src = create_rnd_connection_string(
            length_used, bus_time, node.op.rndam
        )
        ...
        if circuit[idstart].op is None:
            ...
            if circuit[idstart].op.src0 is not None:
                flag_found = flag_found or search_in_depth(
                    circuit[idstart].op.src0, idsearch, circuit
                )
            if circuit[idstart].op.src1 is not None:
                flag_found = flag_found or search_in_depth(
                    circuit[idstart].op.src1, idsearch, circuit
                )
            return flag_found
...
def compute_level_gate(idstart, gatet, circuit):
    ...
    if node.op is None:
        ...
        if node.op.src0 is not None:
            sum_gate0 = compute_level_gate(node.op.src0, gatet, circuit)
        if node.op.src1 is not None:
            sum_gate1 = compute_level_gate(node.op.src1, gatet, circuit)
...
def create_verilog_IO_ports(circuit, outs, ins, interface):
    ...
    for inp in ins:
        ...
        if circuit[inp].dtype == "data":
            IO_ports_str += (
                '(* fv_type="sharing", fv_latency={}, fv_count=1 *)\n'.format(
                    circuit[inp].time
                )
            )
...
def create_rnd_busses(circuit, outs, ins, rnd_busses, validity_pipeline):
    ...
    for rbi, rb in zip(rnd_busses.keys(), rnd_busses.values()):
        ...
        busnode = NodeVar(bus_str, None, 0, "random", create_len_string(rb["lengths"]))
...
def create_verilog_netlist(circuit, outs, ins, mod_name, gen_enable, outfile):
    ...
    top_v_interface = create_top_verilog_interface(
        mod_name, circuit, outs, ins, gen_enable
    )
    ...
    for g in nodelist:
        if g not in outs and g not in ins:
            nentry = create_verilog_template_entry(circuit[g], None, gen_en=gen_enable)
            netlist_txt += nentry + "\n"
    ...
    # Write the netlist
    print("Write netlist in '{}'".format(outfile))
...
def fix_latency_ctrl_mux(muxvar, circuit):
    ...
    if cnode_id not in circuit:
        ...
        # Fix delay
        add_delay_opin(circuit[cnode_id].op, 0, mtime, circuit, ntype="control")
...
def write_stats(circuit, outs, ins, outstats):
    stats = {
        "RNG hpc2": sum(
            int(circuit[inp].lenstr.split("*")[0])
            for inp in ins
            if circuit[inp].dtype == "random"
        ),
...
def build_transformed_verilog_netlist(
    toplevel_fn, convdic, module_name, gen_ctrl, gen_enable, outfile, outstats
):
    print("### START PROCESS ###")
    ...
    print(
        "A total of {} registers have been added".format(
    ...
    create_verilog_netlist(
        circ, outs, ins, mod_name=module_name, gen_enable=gen_enable, outfile=outfile
    )
...
if __name__ == "__main__":
    ...
    if args.verilog_module is None:
        args.verilog_module = args.circuit.stem
