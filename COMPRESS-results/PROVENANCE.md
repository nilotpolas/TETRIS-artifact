...
| SAT solver | Google OR-Tools CP-SAT (bundled with COMPRESS's Python deps) |
...
|---|---|---|
| Boyer-Peralta AES S-box | `compress_source/circuits/aes_bp.txt` | `aes_base/`, `aes_sep/`, `aes_opt/` |
| Canright AES S-box | (Canright variant) | `canright_aes_sbox_opt/` |
| SKINNY S-box | `compress_source/circuits/skinny8.txt` | `skinny_base/`, `skinny_sep/`, `skinny_opt/` |

## COMPRESS modes

- **Base**: straightforward COMPRESS output using `gadget_library/gadgets_base.toml`
- **Sep**: with separation heuristics, `gadgets_sep.toml`
- **Opt**: fully optimized, `gadgets_opt.toml` — the strongest baseline TETRIS is compared against

The three `.toml` files that define each mode are in `compress_source/gadget_library/`.
...
**Note the offset:** COMPRESS uses `d` for number of shares. TETRIS paper uses `d-1` for masking order. So COMPRESS's `d=2` = TETRIS's `d-1=1` (first order), COMPRESS's `d=5` = TETRIS's `d-1=4` (fourth order).
...
```bash
# 1. Clone (or use the bundled compress_source/ directly)
...
Solve times scale sharply with masking order and latency budget:

| Config | Approximate solve time |
|---|---|
| d-1=1, l=4 | 1–3 seconds |
| d-1=1, l=6 | 17 seconds (Boyer-Peralta Opt mode) |
| d-1=1, l=10 | 400+ seconds |
| d-1=4, l=4 | 100+ seconds |
| d-1=4, l=6 | 500+ seconds |

Full experiment matrix (89 configurations) takes roughly **1–3 hours** of wall-clock time on a modern CPU.

## Cross-verifying against the paper

To verify a specific number:

1. Locate the paper table entry, e.g., "BoyerPeralta, d-1=1, lat=6, COMPRESS:Opt column"
2. Find the corresponding file: `aes_opt__aes_bp_d2_l6.json` (note: paper's d-1=1 → COMPRESS's d=2)
3. Open the JSON, read `RNG Bits`, `Area Estimate (GE)`, and `solve_time`
4. Compare with the paper table row

`parse_compress_logs.py` automates this cross-reference; `compare_with_tetris.py` puts it side-by-side with TETRIS's own numbers.

## Verified data point

**BoyerPeralta d-1=1, lat=6, COMPRESS:Opt:**

```
$ cat compress_logs/aes_opt__aes_bp_d2_l6.json | python3 -c 'import json,sys; d=json.load(sys.stdin); print(f"RNG={d[\"RNG Bits\"]} area={d[\"Area Estimate (GE)\"]:.1f} solve={d[\"solve_time\"]:.2f}s")'
RNG=34 area=4630.8 solve=17.43s
```

Paper's Table 2 quotes COMPRESS:Opt at 17.43 seconds for this configuration. TETRIS at the same config is 0.055 seconds → 316.9× speedup.
